...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderTreatmentDetails(treatmentDetails, treatmentName) {
        ...
        const doctorsHtml = treatmentDetails.doctors && treatmentDetails.doctors.length > 0 
            ? treatmentDetails.doctors.map(doctor => {
                ...
                const hasProfilePic = doctor.profilePictureUrl && doctor.profilePictureUrl.trim().length > 0;
        ...
        const imageHtml = treatmentImageUrl 
            ...
            : `<div class="treatment-icon-placeholder"><i class="${getTreatmentIcon(treatmentName)}"></i></div>`;
        ...
        if (makeAppointmentBtn) {
            makeAppointmentBtn.addEventListener('click', () => {
                window.location.href = `/Patient/Appointments/Book?treatmentId=${treatmentDetails.id || ''}&treatmentName=${encodeURIComponent(treatmentDetails.name || '')}`;
            });
        }
    }
    ...
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape' && treatmentDetailsOverlay.classList.contains('is-visible')) {
            closeTreatmentDetails();
        }
    });
});